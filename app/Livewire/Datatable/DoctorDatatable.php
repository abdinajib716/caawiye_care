...
{
    public string $model = Doctor::class;

    public bool $showFilters = false;
    public array $relationships = ['hospital'];

    public array $searchableColumns = ['name', 'specialization', 'phone', 'email'];
    ...
    public function renderHospitalColumn($doctor): string
    {
        if ($doctor->hospital) {
            return '<div class="text-sm font-medium text-blue-600">' . e($doctor->hospital->name) . '</div>';
        }

        return '<span class="text-gray-400">-</span>';
    }

    public function renderContactColumn($doctor): string
    {
        $html = '<div class="text-sm">';

        if ($doctor->phone) {
            $html .= '<div class="font-medium text-gray-900">' . e($doctor->phone) . '</div>';
        }

        if ($doctor->email) {
            $html .= '<div class="text-xs text-gray-500">' . e($doctor->email) . '</div>';
        }
    ...
    {
        $colorClass = match ($doctor->status) {
    ...
    public function renderActionsColumn($doctor): string
    {
        $html = '<div class="flex items-center space-x-2">';

        // View button
        $html .= '<a href="' . route('admin.doctors.show', $doctor) . '" class="inline-flex items-center justify-center w-8 h-8 text-blue-600 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-100 hover:text-blue-700 transition-colors duration-200" title="' . __('View Doctor') . '">';
        $html .= '<iconify-icon icon="lucide:eye" class="w-4 h-4"></iconify-icon>';
        $html .= '</a>';

        // Edit button
        $html .= '<a href="' . route('admin.doctors.edit', $doctor) . '" class="inline-flex items-center justify-center w-8 h-8 text-green-600 bg-green-50 border border-green-200 rounded-lg hover:bg-green-100 hover:text-green-700 transition-colors duration-200" title="' . __('Edit Doctor') . '">';
        ...
        $html .= '<button @click="deleteModalOpen = true" class="inline-flex items-center justify-center w-8 h-8 text-red-600 bg-red-50 border border-red-200 rounded-lg hover:bg-red-100 hover:text-red-700 transition-colors duration-200" title="' . __('Delete Doctor') . '">';
        ...
        $html .= '<h3 class="font-semibold tracking-wide text-gray-700 dark:text-white">' . __('Delete Doctor') . '</h3>';
        ...
        $html .= '<p class="text-gray-500 dark:text-gray-300">' . __('Are you sure you want to delete this doctor?') . '</p>';
        $html .= '<p class="font-medium text-gray-900 dark:text-white mt-2">' . e($doctor->name) . '</p>';
    ...
    protected function getFilterOptions(): array
    {
        ...
        // Add hospitals for filter dropdown
        $options['hospital_id'] = Hospital::query()
            ->active()
            ->orderBy('name')
            ->pluck('name', 'id')
            ->toArray();
