<div class="space-y-6" x-data="labTestBooking()">
    ...
    {{-- Step 2: Select Lab Tests & Patient Info --}}
    <div x-show="currentStepNum === 2" class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
        <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-6">{{ __('Select Lab Tests & Patient Info') }}</h3>
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="form-label">{{ __('Patient Address') }}</label>
                            <input type="text" wire:model="patientAddress" class="form-control" placeholder="{{ __('Enter patient address') }}">
                        </div>
                    </div>

                    {{-- Lab Tests Selection --}}
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">{{ __('Select Lab Tests') }}</label>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-96 overflow-y-auto">
                            @foreach($labTests as $test)
                                <div wire:click="toggleTest({{ $test['id'] }})" 
                                    class="border {{ in_array($test['id'], $selectedTests) ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700' }} rounded-md p-4 cursor-pointer hover:border-blue-400 dark:hover:border-blue-600 transition">
                                    <div class="flex items-start justify-between">
                                        <div class="flex-1">
                                            <div class="font-medium text-gray-900 dark:text-white">{{ $test['name'] }}</div>
                                            <div class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $test['provider']['name'] ?? '' }}</div>
                                            <div class="text-sm font-semibold text-blue-600 dark:text-blue-400 mt-2">${{ number_format($test['cost'], 2) }}</div>
                                        </div>
                    ...
                    @if(!empty($selectedTests))
                        <div class="bg-gray-50 dark:bg-gray-900 rounded-md p-4">
                            <div class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('Selected Tests') }}: {{ count($selectedTests) }}</div>
                            <div class="space-y-1">
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-600 dark:text-gray-400">{{ __('Test Cost') }}</span>
                                    <span class="font-semibold text-gray-900 dark:text-white">${{ number_format($totalCost, 2) }}</span>
                                </div>
                                @if($billCustomerCommission > 0)
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-600 dark:text-gray-400">{{ __('Commission (Bill Customer)') }}</span>
                                    <span class="font-semibold text-green-600 dark:text-green-400">+${{ number_format($billCustomerCommission, 2) }}</span>
                                </div>
                                @endif
                                @if($billProviderCommission > 0)
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-600 dark:text-gray-400">{{ __('Commission (Bill Provider)') }}</span>
                                    <span class="font-semibold text-orange-600 dark:text-orange-400">${{ number_format($billProviderCommission, 2) }}</span>
                                </div>
                                @endif
    ...
    <div x-show="currentStepNum === 3" class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
        <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-6">{{ __('Review & Assign Nurse') }}</h3>
                
                <div class="space-y-6">
                    {{-- Assign Nurse --}}
                    <div>
                        <label class="form-label">{{ __('Assign Nurse (Optional)') }}</label>
                        <select wire:model="nurseId" class="form-control w-full">
                    ...
                    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-md p-6">
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                <span class="text-blue-700 dark:text-blue-300">{{ __('Selected Tests') }}</span>
                                <span class="font-semibold text-blue-900 dark:text-blue-100">{{ count($selectedTests) }} {{ __('tests') }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-blue-700 dark:text-blue-300">{{ __('Test Cost') }}</span>
                                <span class="font-semibold text-blue-900 dark:text-blue-100">${{ number_format($totalCost, 2) }}</span>
                            </div>
                            @if($billCustomerCommission > 0)
                            <div class="flex justify-between">
                                <span class="text-blue-700 dark:text-blue-300">{{ __('Commission (Bill Customer)') }}</span>
                                <span class="font-semibold text-green-600 dark:text-green-400">+${{ number_format($billCustomerCommission, 2) }}</span>
                            </div>
                            @endif
                            @if($billProviderCommission > 0)
                            <div class="flex justify-between">
                                <span class="text-blue-700 dark:text-blue-300">{{ __('Commission (Bill Provider)') }}</span>
                                <span class="font-semibold text-orange-600 dark:text-orange-400">${{ number_format($billProviderCommission, 2) }}</span>
                            </div>
                            @endif
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <a x-show="currentStepNum === 1" href="{{ route('admin.lab-test-bookings.index') }}" class="btn btn-secondary">
        ...
        function labTestBooking() {
            return {
                ...
                steps: [
                    ...
                    { label: '{{ __("Lab Tests") }}', description: '{{ __("Choose tests") }}' },
