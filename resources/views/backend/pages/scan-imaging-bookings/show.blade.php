<x-layouts.backend-layout :breadcrumbs="$breadcrumbs">
    <div class="space-y-6">
            ...
            <div class="p-6">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Scan & Imaging Booking') }}</p>
                    </div>
                ...
                @if($booking->status === 'pending')
                    <div class="flex items-center space-x-2 mt-4">
                        @can('scan_imaging_booking.edit')
                            <form method="POST" action="{{ route('admin.scan-imaging-bookings.confirm', $booking) }}">
                                @csrf
                            ...
                            <form method="POST" action="{{ route('admin.scan-imaging-bookings.cancel', $booking) }}"
                ...
                @elseif($booking->status === 'confirmed')
                    <div class="flex items-center space-x-2 mt-4">
                        @can('scan_imaging_booking.edit')
                            <form method="POST" action="{{ route('admin.scan-imaging-bookings.mark-in-progress', $booking) }}">
                                @csrf
                ...
                @elseif($booking->status === 'in_progress')
                    <div class="flex items-center space-x-2 mt-4">
                        @can('scan_imaging_booking.edit')
                            <form method="POST" action="{{ route('admin.scan-imaging-bookings.complete', $booking) }}">
                                @csrf
            ...
            <x-card class="bg-white dark:bg-gray-800">
                <div class="p-6">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700 pb-2 mb-4">
                        {{ __('Customer & Service Information') }}
                    </h3>
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <p class="mt-1 text-gray-900 dark:text-white">
                                ...
                                @else
                                    <span class="text-gray-400 dark:text-gray-500">{{ __('Deleted customer') }}</span>
                                @endif
                            </p>
                        </div>
                        ...
                        <div>
                            <label class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('Service') }}</label>
                            <p class="mt-1 text-gray-900 dark:text-white">
                                @if($booking->scanImagingService)
                                    <a href="{{ route('admin.scan-imaging-services.show', $booking->scanImagingService) }}" class="text-blue-600 hover:text-blue-800 dark:text-blue-400">
                                        {{ $booking->service_name }}
                                    </a>
                                @else
                                    <span class="text-gray-400 dark:text-gray-500">{{ $booking->service_name }}</span>
                                @endif
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-gray-900 dark:text-white">
                                @if($booking->provider)
                                    <a href="{{ route('admin.providers.show', $booking->provider) }}" class="text-blue-600 hover:text-blue-800 dark:text-blue-400">
                                        {{ $booking->provider->name }}
                                    </a>
                                @else
                                    <span class="text-gray-400 dark:text-gray-500">{{ __('Provider unavailable') }}</span>
                                @endif
                            </p>
                        </div>

                        <div>
                            <label class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('Appointment Time') }}</label>
                            <p class="mt-1 text-gray-900 dark:text-white">{{ $booking->appointment_time->format('M d, Y h:i A') }}</p>
                        </div>
                    </div>
                </div>
            </x-card>
                ...
                <div class="p-6">
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between">
                            <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('Service Cost') }}</span>
                            <span class="font-medium text-gray-900 dark:text-white">{{ $booking->cost_formatted }}</span>
                        </div>
        ...
        <!-- Actions -->
        <div class="flex items-center justify-between">
            <a href="{{ route('admin.scan-imaging-bookings.index') }}"
