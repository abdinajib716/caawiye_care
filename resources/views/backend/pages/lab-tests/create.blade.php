<x-layouts.backend-layout :breadcrumbs="$breadcrumbs">
    <x-card>
        <x-slot name="header">
            <h3 class="text-lg font-medium text-gray-900">{{ __('Create New Lab Test') }}</h3>
        </x-slot>

        <form method="POST" action="{{ route('admin.lab-tests.store') }}" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="commission_type" id="commission_type" required class="form-control">
                        <option value="bill_provider" {{ old('commission_type') === 'bill_provider' ? 'selected' : '' }}>{{ __('Bill Provider (Deduct from Provider)') }}</option>
                        <option value="bill_customer" {{ old('commission_type') === 'bill_customer' ? 'selected' : '' }}>{{ __('Bill Customer (Add to Customer)') }}</option>
                    </select>
