        ...
        <x-card class="bg-white">
            <x-slot name="header">
                <div class="flex items-center justify-between">
                    <h3 class="text-lg font-medium text-gray-900">{{ __('Edit Customer') }}</h3>
            ...
            <form action="{{ route('admin.customers.update', $customer) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="rounded-lg bg-gray-50 p-4">
                            <h4 class="text-sm font-medium text-gray-900 mb-3">{{ __('Customer Details') }}</h4>
                            <dl class="space-y-2">
                                <div class="flex justify-between text-sm">
                                    <dt class="text-gray-500">{{ __('Customer ID') }}</dt>
                                    <dd class="text-gray-900 font-medium">#{{ $customer->id }}</dd>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <dt class="text-gray-500">{{ __('Current Phone') }}</dt>
                                    <dd class="text-gray-900 font-medium">{{ $customer->formatted_phone }}</dd>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <dt class="text-gray-500">{{ __('Created') }}</dt>
                                    <dd class="text-gray-900">{{ $customer->created_at->format('M d, Y') }}</dd>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <dt class="text-gray-500">{{ __('Last Updated') }}</dt>
                                    <dd class="text-gray-900">{{ $customer->updated_at->format('M d, Y') }}</dd>
                                </div>
                            </dl>
                        </div>
                    </div>
                ...
                <div class="flex items-center justify-end space-x-3 border-t border-gray-200 pt-6">
                    <x-buttons.button variant="secondary" as="a" href="{{ route('admin.customers.show', $customer) }}">
                        {{ __('Cancel') }}
                    </x-buttons.button>
