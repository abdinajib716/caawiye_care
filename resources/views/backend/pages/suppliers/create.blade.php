<x-layouts.backend-layout :breadcrumbs="$breadcrumbs">
    <div class="mx-auto max-w-2xl">
        <x-card>
            <x-slot name="header">
                <h3 class="text-lg font-medium text-gray-900 dark:text-white">{{ __('Add New Supplier') }}</h3>
            </x-slot>
            ...
            <form action="{{ route('admin.suppliers.store') }}" method="POST">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" class="form-control @error('name') border-red-500 @enderror" value="{{ old('name') }}" required>
                        @error('name')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>

                    <div>
                        <label for="phone" class="form-label">{{ __('Phone Number') }} <span class="text-red-500">*</span></label>
                        <input type="tel" name="phone" id="phone" class="form-control @error('phone') border-red-500 @enderror" value="{{ old('phone') }}" required>
                        @error('phone')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>

                    <div>
                        <label for="email" class="form-label">{{ __('Email') }}</label>
                        <input type="email" name="email" id="email" class="form-control @error('email') border-red-500 @enderror" value="{{ old('email') }}">
